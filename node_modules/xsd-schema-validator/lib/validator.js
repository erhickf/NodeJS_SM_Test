 ...
 */
Validator.prototype.validateXML = function(propsObj, callback) {
  var cwd = this.cwd,
      debug = this.debug;
  ...
  withValidator(function(err) {
    ...
    if (err) {
      return callback(err, {
		...
		request:propsObj.request,
		response:propsObj.response,
		jsonData:propsObj.jsonData,
		schema:propsObj.schema
      });
    }
...
module.exports.validateXML = function(propsObj,callback) {  return new Validator().validateXML(propsObj,callback); };